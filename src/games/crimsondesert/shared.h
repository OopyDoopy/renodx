...
#define MICRO_SHADOW_QUALITY                   (RR_ENABLED == 1.f ? ((float)((((CUSTOM_FLAGS_AS_UINT & CUSTOM_FLAGS__MICRO_SHADOW_QUALITY_BIT0) != 0u) ? 1u : 0u) | (((CUSTOM_FLAGS_AS_UINT & CUSTOM_FLAGS__MICRO_SHADOW_QUALITY_BIT1) != 0u) ? 2u : 0u))) : MICRO_SHADOW_QUALITY_OFF)
...
#define CONTACT_MICRO_RANGE_NEAR               0.12f
#define CONTACT_MICRO_RANGE_FAR                0.80f
#define CONTACT_MICRO_DISTANCE_FADE            saturate(mad(-0.025f, MICRO_LINEAR_DEPTH, 3.0f))
...
#define CONTACT_SHADOW_RT_MARCH_SAMPLES        (CONTACT_SHADOW_IS_FULL ? 32.0f : (CONTACT_SHADOW_IS_BALANCED ? 24.0f : 16.0f))
#define CONTACT_SHADOW_RT_REACH_TARGET         (CONTACT_SHADOW_IS_FULL ? 0.115f : (CONTACT_SHADOW_IS_BALANCED ? 0.080f : 0.050f))
#define CONTACT_SHADOW_RT_START_TARGET         (CONTACT_SHADOW_IS_FULL ? 0.55f : (CONTACT_SHADOW_IS_BALANCED ? 1.00f : 2.00f))
#define CONTACT_SHADOW_RT_ACCUM_STRENGTH       (CONTACT_SHADOW_IS_FULL ? 2.35f : (CONTACT_SHADOW_IS_BALANCED ? 1.75f : (CONTACT_SHADOW_IS_LOW ? 1.15f : 1.0f)))
#define CONTACT_SHADOW_RT_FINAL_STRENGTH       (CONTACT_SHADOW_IS_FULL ? 1.65f : (CONTACT_SHADOW_IS_BALANCED ? 1.35f : (CONTACT_SHADOW_IS_LOW ? 1.10f : 1.0f)))
#define CONTACT_MICRO_DETAIL_STRENGTH_RT       (CONTACT_SHADOW_IS_FULL ? 1.35f : (CONTACT_SHADOW_IS_BALANCED ? 1.00f : (CONTACT_SHADOW_IS_LOW ? 0.65f : 0.f)))
#define CONTACT_MICRO_RANGE_NEAR_RT            (CONTACT_SHADOW_IS_FULL ? 0.16f : 0.12f)
#define CONTACT_MICRO_RANGE_FAR_RT             (CONTACT_SHADOW_IS_FULL ? 1.15f : (CONTACT_SHADOW_IS_BALANCED ? 0.80f : 0.65f))
#define CONTACT_MICRO_DISTANCE_FADE_RT         saturate(mad((CONTACT_SHADOW_IS_FULL ? -0.020f : -0.025f), MICRO_LINEAR_DEPTH, (CONTACT_SHADOW_IS_FULL ? 3.35f : 3.0f)))
#define CONTACT_MICRO_THICKNESS_MULTIPLIER_RT  (CONTACT_SHADOW_IS_FULL ? 0.58f : 0.50f)
#define CONTACT_MICRO_OCCLUSION_SCALE_RT       (CONTACT_SHADOW_IS_FULL ? 3.25f : (CONTACT_SHADOW_IS_BALANCED ? 2.50f : 2.00f))
