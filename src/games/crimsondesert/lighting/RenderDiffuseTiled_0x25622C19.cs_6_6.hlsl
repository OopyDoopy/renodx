...
void main(
  ...
  if (!(((_rndx_quadInvalid0 && _rndx_quadInvalid1) && _rndx_quadInvalid2) && _rndx_quadInvalid3)) {
    ...
    // Description: RenderDiffuseTiledCS 0x25622C19 applies AO+ foliage color shaping to foliage stencil materials after vanilla resolves the direct diffuse base color, using g_sceneShadowColor visibility so shadowed foliage does not receive the fully sunlit color-shaping path.
    ...
    // Description: RenderDiffuseTiledCS 0x25622C19 adds AO+ foliage transmission for foliage stencil materials during direct diffuse lighting, injecting the helper's transmission contribution into the final diffuse color while preserving vanilla lighting for non-foliage pixels and lower foliage modes.
    ...
    // Description: RenderDiffuseTiledCS 0x25622C19 applies RenoDX foliage AO darkening to the final direct-lit scene color for foliage stencil materials, using vanilla scene AO and shadow visibility so direct light remains naturally occluded.
