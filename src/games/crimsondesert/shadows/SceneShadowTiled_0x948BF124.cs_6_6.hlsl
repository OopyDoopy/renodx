...
void main(
  ...
  if ((_79 < 1.0000000116860974e-07f) || _107) {
    ...
    if (_2284 > 0.0f) {
      ...
      // Description: SceneShadowTiledCS 0x948BF124 uses this native contact ray direction on Lighting High/Ultra Sun/Moon RT lanes; stable direction keeps Contact Micro Shadows from inheriting per-frame random cone wobble when the feature is enabled.
      ...
      // RenoDX: >>> [Patch: ContactMicroShadowsFamily] [Version: 1.10]
      // Description: SceneShadowTiledCS 0x948BF124 raises native RT contact-ray samples only while Contact Micro Shadows is enabled, preserving the vanilla sample count when Off.
      ...
      // RenoDX: >>> [Patch: ContactMicroShadowsFamily] [Version: 1.10]
      // Description: SceneShadowTiledCS 0x948BF124 extends RT contact reach gradually with Contact Micro Shadows quality; Off evaluates to the vanilla reach.
      ...
      if (_2296 || ((_80 != 15) && ((uint)(_80 + -12) < (uint)7))) {
        // RenoDX: >>> [Patch: ContactMicroShadowsFamily] [Version: 1.10]
        // Description: SceneShadowTiledCS 0x948BF124 moves the native RT contact start closer only while Contact Micro Shadows is enabled, preserving vanilla start distance when Off.
        _2621 = (_2608 * lerp(10.0f, CONTACT_SHADOW_RT_START_TARGET, CONTACT_SHADOW_RT_TUNING));
        // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
      } else {
        if (_80 == 15) {
          // RenoDX: >>> [Patch: ContactMicroShadowsFamily] [Version: 1.10]
          // Description: SceneShadowTiledCS 0x948BF124 applies the same Contact Micro Shadows RT start-distance blend to the depth-faded material branch.
  ...
  // [sem: _3__36__0__0__g_depthStencil_load]
        ...
        if (!(_2883 == 0)) {
          ...
          _2925 = _2828;
          // RenoDX: >>> [Patch: ContactMicroShadowsFamily] [Version: 1.10]
          // Description: SceneShadowTiledCS 0x948BF124 boosts native RT contact accumulation with Contact Micro Shadows quality so the helper and native RT contact terms share the same strength curve.
  ...
  // [sem: _3__36__0__0__g_baseColor_load]
    ...
    // RenoDX: >>> [Patch: ContactMicroShadowsFamily] [Version: 1.10]
    // Description: SceneShadowTiledCS 0x948BF124 fills sub-pixel occluders in the Lighting High/Ultra Sun/Moon RT shadow sibling, then applies the existing RT final-strength blend and edge fade.
