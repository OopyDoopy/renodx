...
void main(
  ...
  // [sem: _3__36__0__0__g_dynamicShadowColorArray_sampleLod]
    ...
    if (_1272 > 0.0f) {
      ...
      // Description: SceneShadowTiledCS 0xC224C706 uses this native contact ray direction on Lighting High RT-off lanes; stable direction keeps Contact Micro Shadows from inheriting per-frame random cone wobble when the feature is enabled.
      ...
      // RenoDX: >>> [Patch: ContactMicroShadowsFamily] [Version: 1.10]
      // Description: SceneShadowTiledCS 0xC224C706 raises native contact-ray samples only while Contact Micro Shadows is enabled, preserving the vanilla sample count when Off.
      ...
      // RenoDX: >>> [Patch: ContactMicroShadowsFamily] [Version: 1.10]
      // Description: SceneShadowTiledCS 0xC224C706 extends the native non-RT contact reach gradually with the Contact Micro Shadows quality value; Off evaluates to the vanilla reach.
      ...
      if (_1284 || ((_78 != 15) && ((uint)(_78 + -12) < (uint)7))) {
        // RenoDX: >>> [Patch: ContactMicroShadowsFamily] [Version: 1.10]
        // Description: SceneShadowTiledCS 0xC224C706 moves the native contact start closer only while Contact Micro Shadows is enabled, preserving vanilla start distance when Off.
        _1472 = (_1459 * lerp(10.0f, 2.0f, CONTACT_SHADOW_START_BLEND));
        // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
      } else {
        if (_78 == 15) {
          // RenoDX: >>> [Patch: ContactMicroShadowsFamily] [Version: 1.10]
          // Description: SceneShadowTiledCS 0xC224C706 applies the same Contact Micro Shadows start-distance blend to the depth-faded material branch.
  ...
  // [sem: _3__36__0__0__g_depthStencil_load]
        ...
        if (!(_1732 == 0)) {
          ...
          _1774 = _1677;
          // RenoDX: >>> [Patch: ContactMicroShadowsFamily] [Version: 1.10]
          // Description: SceneShadowTiledCS 0xC224C706 softens native contact accumulation with Contact Micro Shadows tuning so the added detail blends with the shipped non-RT contact result.
  ...
  // [sem: _3__36__0__0__g_baseColor_load]
    ...
    // RenoDX: >>> [Patch: ContactMicroShadowsFamily] [Version: 1.10]
    // Description: SceneShadowTiledCS 0xC224C706 fills sub-pixel occluders in the Lighting High RT-off shadow sibling, then fades the helper near screen edges to avoid bright edge artifacts.
