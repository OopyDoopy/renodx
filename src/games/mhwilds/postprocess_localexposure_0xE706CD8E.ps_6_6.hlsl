...
float4 main(
  ...
  if (_50) {
    ...
    if (aberrationEnable == 0) {
      do {
        ...
        do {
#if 0  // Original local-exposure calculation
            ...
            do {
              ...
              do {
                ...
                do {
                  ...
                  _2812 = max((mad(_2795, (fColorMatrix[2].x), mad(_2794, (fColorMatrix[1].x), (_2793 * (fColorMatrix[0].x)))) + (fColorMatrix[3].x)), 0.0f);
                  _2813 = max((mad(_2795, (fColorMatrix[2].y), mad(_2794, (fColorMatrix[1].y), (_2793 * (fColorMatrix[0].y)))) + (fColorMatrix[3].y)), 0.0f);
                  _2814 = max((mad(_2795, (fColorMatrix[2].z), mad(_2794, (fColorMatrix[1].z), (_2793 * (fColorMatrix[0].z)))) + (fColorMatrix[3].z)), 0.0f);
                } while (false);
              } while (false);
            } while (false);
          } while (false);
...
#endif
  float _2815 = min(_2812, 65000.0f);
  float _2816 = min(_2813, 65000.0f);
  float _2817 = min(_2814, 65000.0f);
  bool _2820 = isfinite(max(max(_2815, _2816), _2817));
  float _2821 = select(_2820, _2815, 1.0f);
  float _2822 = select(_2820, _2816, 1.0f);
  float _2823 = select(_2820, _2817, 1.0f);
  if (!((cPassEnabled & 8) == 0)) {
    _2858 = saturate(((cvdR.x * _2821) + (cvdR.y * _2822)) + (cvdR.z * _2823));
    _2859 = saturate(((cvdG.x * _2821) + (cvdG.y * _2822)) + (cvdG.z * _2823));
    _2860 = saturate(((cvdB.x * _2821) + (cvdB.y * _2822)) + (cvdB.z * _2823));
  } else {
    _2858 = _2821;
    _2859 = _2822;
    _2860 = _2823;
  ...
  if (!((cPassEnabled & 16) == 0)) {
    float _2875 = screenInverseSize.x * SV_Position.x;
    float _2876 = screenInverseSize.y * SV_Position.y;
    float4 _2879 = ImagePlameBase.SampleLevel(BilinearClamp, float2(_2875, _2876), 0.0f);
    float _2884 = _2879.x * ColorParam.x;
    float _2885 = _2879.y * ColorParam.y;
    float _2886 = _2879.z * ColorParam.z;
    float _2889 = ImagePlameAlpha.SampleLevel(BilinearClamp, float2(_2875, _2876), 0.0f);
    float _2894 = (_2879.w * ColorParam.w) * saturate((_2889.x * Levels_Rate) + Levels_Range);
    do {
      if (_2884 < 0.5f) {
        _2906 = ((_2858 * 2.0f) * _2884);
      } else {
        _2906 = (1.0f - (((1.0f - _2858) * 2.0f) * (1.0f - _2884)));
      }
      do {
        if (_2885 < 0.5f) {
          _2918 = ((_2859 * 2.0f) * _2885);
        } else {
          _2918 = (1.0f - (((1.0f - _2859) * 2.0f) * (1.0f - _2885)));
        }
        do {
          if (_2886 < 0.5f) {
            _2930 = ((_2860 * 2.0f) * _2886);
          } else {
            _2930 = (1.0f - (((1.0f - _2860) * 2.0f) * (1.0f - _2886)));
          }
          _2941 = (lerp(_2858, _2906, _2894));
          _2942 = (lerp(_2859, _2918, _2894));
          _2943 = (lerp(_2860, _2930, _2894));
        } while (false);
      } while (false);
    } while (false);
  } else {
  ...
  if (tonemapParam_isHDRMode == 0.0f) {
    float _2951 = invLinearBegin * _2941;
    do {
      if (!(_2941 >= linearBegin)) {
        _2959 = ((_2951 * _2951) * (3.0f - (_2951 * 2.0f)));
      } else {
        _2959 = 1.0f;
      }
      float _2960 = invLinearBegin * _2942;
      do {
        if (!(_2942 >= linearBegin)) {
          _2968 = ((_2960 * _2960) * (3.0f - (_2960 * 2.0f)));
        } else {
          _2968 = 1.0f;
        }
        float _2969 = invLinearBegin * _2943;
        do {
          if (!(_2943 >= linearBegin)) {
            _2977 = ((_2969 * _2969) * (3.0f - (_2969 * 2.0f)));
          } else {
            _2977 = 1.0f;
          }
          float _2986 = select((_2941 < linearStart), 0.0f, 1.0f);
          float _2987 = select((_2942 < linearStart), 0.0f, 1.0f);
          float _2988 = select((_2943 < linearStart), 0.0f, 1.0f);
          _3048 = (((((contrast * _2941) + madLinearStartContrastFactor) * (_2959 - _2986)) + (((pow(_2951, toe)) * (1.0f - _2959)) * linearBegin)) + ((maxNit - (exp2((contrastFactor * _2941) + mulLinearStartContrastFactor) * displayMaxNitSubContrastFactor)) * _2986));
          _3049 = (((((contrast * _2942) + madLinearStartContrastFactor) * (_2968 - _2987)) + (((pow(_2960, toe)) * (1.0f - _2968)) * linearBegin)) + ((maxNit - (exp2((contrastFactor * _2942) + mulLinearStartContrastFactor) * displayMaxNitSubContrastFactor)) * _2987));
          _3050 = (((((contrast * _2943) + madLinearStartContrastFactor) * (_2977 - _2988)) + (((pow(_2969, toe)) * (1.0f - _2977)) * linearBegin)) + ((maxNit - (exp2((contrastFactor * _2943) + mulLinearStartContrastFactor) * displayMaxNitSubContrastFactor)) * _2988));
        } while (false);
      } while (false);
    } while (false);
  } else {
  ...
  SV_Target.xyz = CustomTonemap(float3(_2941, _2942, _2943), params, tonemapParam_isHDRMode == 0.0f);
  SV_Target.w = 0.0f;
  return SV_Target;
}
