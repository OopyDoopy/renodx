...
//   GamutCompression(ungraded_compressed, compression_scale);
  ...
  if (CUSTOM_LUT_SCALING != 0.f) {
    ...
    r3.xyz = ApplyLUTScaling(
      ...
      lut_black,
      lut_mid,
        Sample(gLinearToGammaRamp, EncodeLUTInput(float3(1.f, 1.f, 1.f)), 16).xyz,
        r1.zwy);
  }
  ...
  graded_hdr = lerp(ungraded, graded_hdr, RENODX_COLOR_GRADE_STRENGTH);
  r3.xyz = CustomTonemap(graded_hdr);
