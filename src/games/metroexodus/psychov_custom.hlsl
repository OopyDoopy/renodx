...
float ContrastSafeUpperSpline(float x, float contrast, float mid_gray = 0.18f, float upper_spline_reduction = 0.35f) {
  if (contrast == 1.f) return x;

  float contrasted = renodx::color::grade::ContrastSafe(x, contrast, mid_gray);
  float normalized = renodx::math::DivideSafe(max(x, 0.f), mid_gray, 0.f);
  float upper_spline = smoothstep(1.f, 2.f, normalized);
  float effect = 1.f - upper_spline * saturate(upper_spline_reduction);

  return max(0.f, x + (contrasted - x) * effect);
}

float3 ContrastSafeUpperSpline(float3 x, float contrast, float3 mid_gray = 0.18f, float upper_spline_reduction = 0.35f) {
  if (contrast == 1.f) return x;

  float3 contrasted = renodx::math::SignPow(x / mid_gray, contrast) * mid_gray;
  float3 normalized = renodx::math::DivideSafe(max(x, 0.f), mid_gray, 0.f.xxx);
  float3 upper_spline = smoothstep(
      1.f.xxx,
      2.f.xxx,
      normalized);
  float3 effect = 1.f.xxx - upper_spline * saturate(upper_spline_reduction);
...
struct Config {
  ...
  float shadows;
  float contrast;
  float upper_spline_contrast;
  float contrast_upper_spline_reduction;
  float purity_scale;
...
Config Create() {
  ...
  config.shadows = 1.f;
  config.contrast = 1.f;
  config.upper_spline_contrast = 1.f;
  config.contrast_upper_spline_reduction = 0.35f;
...
#define RENODX_PSYCHOV_CLIP_POINT 100.f
#endif
  config.clip_point = RENODX_PSYCHOV_CLIP_POINT;

#if !defined(RENODX_PSYCHOV_HUE_RESTORE)
#define RENODX_PSYCHOV_HUE_RESTORE 1.f
#endif
  config.hue_restore = RENODX_PSYCHOV_HUE_RESTORE;

#if !defined(RENODX_PSYCHOV_ADAPTATION_CONTRAST)
#define RENODX_PSYCHOV_ADAPTATION_CONTRAST 1.f
#endif
  config.adaptation_contrast = RENODX_PSYCHOV_ADAPTATION_CONTRAST;

#if !defined(RENODX_PSYCHOV_GRADING_MODE)
#define RENODX_PSYCHOV_GRADING_MODE 0
#endif
  config.grading_mode = RENODX_PSYCHOV_GRADING_MODE;

#if !defined(RENODX_PSYCHOV_WHITE_CURVE_MODE)
#define RENODX_PSYCHOV_WHITE_CURVE_MODE 0
#endif
  config.white_curve_mode = RENODX_PSYCHOV_WHITE_CURVE_MODE;

#if !defined(RENODX_PSYCHOV_CONE_RESPONSE_EXPONENT)
#define RENODX_PSYCHOV_CONE_RESPONSE_EXPONENT 1.f
#endif
  config.cone_response_exponent = RENODX_PSYCHOV_CONE_RESPONSE_EXPONENT;

#if !defined(RENODX_PSYCHOV_CURRENT_ADAPTIVE_STATE_BT709)
#if defined(RENODX_TONE_MAP_MID_GRAY_IN)
#define RENODX_PSYCHOV_CURRENT_ADAPTIVE_STATE_BT709 RENODX_TONE_MAP_MID_GRAY_IN
#else
#define RENODX_PSYCHOV_CURRENT_ADAPTIVE_STATE_BT709 0.18f
#endif
#endif
  config.current_adaptive_state_bt709 = RENODX_PSYCHOV_CURRENT_ADAPTIVE_STATE_BT709;

#if !defined(RENODX_PSYCHOV_CURRENT_BACKGROUND_STATE_BT709)
#if defined(RENODX_TONE_MAP_MID_GRAY_OUT)
#define RENODX_PSYCHOV_CURRENT_BACKGROUND_STATE_BT709 RENODX_TONE_MAP_MID_GRAY_OUT
#else
#define RENODX_PSYCHOV_CURRENT_BACKGROUND_STATE_BT709 0.18f
#endif
#endif
  config.current_background_state_bt709 = RENODX_PSYCHOV_CURRENT_BACKGROUND_STATE_BT709;

#if !defined(RENODX_PSYCHOV_GAMUT_COMPRESSION)
#define RENODX_PSYCHOV_GAMUT_COMPRESSION 1.f
#endif
  config.gamut_compression = RENODX_PSYCHOV_GAMUT_COMPRESSION;

#if !defined(RENODX_PSYCHOV_GAMUT_COMPRESSION_MODE)
#define RENODX_PSYCHOV_GAMUT_COMPRESSION_MODE 1
#endif
  config.gamut_compression_mode = RENODX_PSYCHOV_GAMUT_COMPRESSION_MODE;

#if !defined(RENODX_PSYCHOV_ADAPTIVE_NORMALIZATION)
#define RENODX_PSYCHOV_ADAPTIVE_NORMALIZATION 1.f
#endif
  config.adaptive_normalization = RENODX_PSYCHOV_ADAPTIVE_NORMALIZATION;
...
float3 psychotm_customized(
  ...
  float upper_spline_contrast = psycho_config.upper_spline_contrast;
  float contrast_upper_spline_reduction = psycho_config.contrast_upper_spline_reduction;
  ...
  if (grading_mode == 0) {
    if (upper_spline_contrast != 1.f) {
      lms_graded = ContrastSafeUpperSpline(
          lms_graded,
          upper_spline_contrast,
          current_adaptive_state_lms,
          contrast_upper_spline_reduction);
    }
    if (highlights != 1.f) {
      if (highlights > 1.f) {
    ...
    // Stage 1: apply UI highlight/shadow/contrast controls in luminosity space.
    if (upper_spline_contrast != 1.f) {
      yf_target = ContrastSafeUpperSpline(
          yf_target,
          upper_spline_contrast,
          yf_midgray,
          contrast_upper_spline_reduction);
