...
// ---- Created with 3Dmigoto v1.4.1 on Mon Jun  8 01:52:54 2026
...
cbuffer global_viewport : register(b0)
{
  ...
  float upscaling_method : packoffset(c45.z);
  float vrs_enabled : packoffset(c45.w);
  float vrs_coverage_enabled : packoffset(c46);
  float vrs_single_axis_opacity_multiplier : packoffset(c46.y);
  float vrs_tile_size : packoffset(c46.z);
  float imp_transparent_override : packoffset(c46.w);
  float debug_rendering : packoffset(c47);
  float post_effects_enabled : packoffset(c47.y);
  float4 raw_non_checkerboarded_viewport : packoffset(c48);
  float debug_lod : packoffset(c49);
  float debug_shadow_lod : packoffset(c49.y);
  float texture_density_visualization : packoffset(c49.z);
}
