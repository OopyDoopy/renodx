#ifndef RENODX_SHADERS_TONEMAP_PSYCHOV_TEST25_CUSTOM_HLSL_
#define RENODX_SHADERS_TONEMAP_PSYCHOV_TEST25_CUSTOM_HLSL_
...
// -----------------------------------------------------------------------------
// Custom Test25 path
// Added from the companion function set while retaining the full Test25 base.
// -----------------------------------------------------------------------------
...
static const int PSYCHOV25_CUSTOM_HUE_RECOVERY_SOURCE_DIRECTION = 0;
static const int PSYCHOV25_CUSTOM_HUE_RECOVERY_SIGNED_OPPONENT = 1;

float PsychoV25CustomSignedOpponentRetention(
  float white_progress,
  float retention_exponent) {
  return 1.f - pow(
...
float3 PsychoV25CustomOpponentACCFromWeightedDelta(
  float3 delta_weighted_lms) {
  float3 neutral_weighted =
    renodx::color::macleod_boynton::WeighLMS(1.f.xxx);
...
float3 PsychoV25CustomWeightedDeltaFromOpponentACC(float3 acc) {
  float3 neutral_weighted =
    renodx::color::macleod_boynton::WeighLMS(1.f.xxx);
  float m_to_l = renodx::math::DivideSafe(
    neutral_weighted.x,
  ...
  float delta_m = renodx::math::DivideSafe(
    acc.x - acc.y,
    1.f + m_to_l,
    0.f);
  float delta_l = acc.x - delta_m;
  float delta_s = renodx::math::DivideSafe(
    acc.z + acc.x,
    s_to_lm,
    0.f);
...
float3 ApplyPsychoV25CustomSignedOpponentRetention(
  ...
  float3 source_neutral = adapted_neutral
              * renodx::math::DivideSafe(
                ...
                adapted_neutral_yf,
                1.f);
  float3 source_acc = PsychoV25CustomOpponentACCFromWeightedDelta(
              source_weighted - source_neutral)
            / source_yf;
  float red_retention = PsychoV25CustomSignedOpponentRetention(
    white_progress,
    retention_exponents.x);
  float green_retention = PsychoV25CustomSignedOpponentRetention(
    white_progress,
    retention_exponents.y);
  float blue_retention = PsychoV25CustomSignedOpponentRetention(
    white_progress,
    retention_exponents.z);
  float yellow_retention = PsychoV25CustomSignedOpponentRetention(
    white_progress,
    retention_exponents.w);
  ...
  float3 compressed_weighted =
    renodx::tonemap::psychov::psycho25_ToAdaptiveRelativeWeightedLMS(
      compressed_lms,
      adaptive_state_lms);
  float target_yf = compressed_weighted.x + compressed_weighted.y;
  float3 peak_weighted =
    renodx::tonemap::psychov::psycho25_ToAdaptiveRelativeWeightedLMS(
      peak_lms,
      adaptive_state_lms);
  float peak_yf = peak_weighted.x + peak_weighted.y;
  if (target_yf <= renodx::tonemap::psychov::PSYCHO25_EPSILON
    || peak_yf <= renodx::tonemap::psychov::PSYCHO25_EPSILON) {
  return compressed_lms;
  }
  ...
  float3 target_neutral = peak_weighted
              * renodx::math::DivideSafe(
                ...
                peak_yf,
                1.f);
  float3 target_delta = PsychoV25CustomWeightedDeltaFromOpponentACC(
    float3(0.f, rg_out * target_yf, yv_out * target_yf));
  float3 output_lms = renodx::color::macleod_boynton::UnweighLMS(
    renodx::tonemap::psychov::psycho25_FromAdaptiveRelativeWeightedLMS(
      target_neutral + target_delta,
      adaptive_state_lms));
  float output_yf =
    renodx::tonemap::psychov::psycho25_YfFromLMS(output_lms);
  if (output_yf <= renodx::tonemap::psychov::PSYCHO25_EPSILON) {
  return compressed_lms;
  }

  return output_lms * renodx::math::DivideSafe(
              renodx::tonemap::psychov::psycho25_YfFromLMS(
                compressed_lms),
              output_yf,
              1.f);
}

  float3 ApplyPsychoV25CustomSignedOpponentRetentionAndGamutCompression(
    float3 source_lms,
    float3 compressed_lms,
    float3 adaptive_state_lms,
    float3 background_state_lms,
    float3 peak_lms,
    float retention_strength,
    float4 retention_exponents,
    bool compress_target_gamut = true) {
    float background_yf =
      renodx::tonemap::psychov::psycho25_YfFromLMS(background_state_lms);
    float peak_yf = renodx::tonemap::psychov::psycho25_YfFromLMS(peak_lms);
    float compression_power =
      renodx::tonemap::psychov::psycho25_AutoCompressionFromCenteredReferenceRange(
        background_yf,
        peak_yf);
    float background_powered = pow(
      max(
        saturate(renodx::math::DivideSafe(
          background_yf,
          peak_yf,
          1.f)),
        renodx::tonemap::psychov::PSYCHO25_EPSILON),
    ...
    float white_progress = saturate(renodx::math::DivideSafe(
      pow(
        ...
        compression_power)
        - background_powered,
      1.f - background_powered,
      0.f));
    float3 retained_lms = ApplyPsychoV25CustomSignedOpponentRetention(
      compressed_lms,
      source_lms,
      adaptive_state_lms,
      peak_lms,
    ...
    float3 output_lms = lerp(
      compressed_lms,
      retained_lms,
      saturate(retention_strength));
    if (!compress_target_gamut) return output_lms;
    return renodx::tonemap::psychov::psycho25_GamutCompressLMSBoundAdaptive(
      output_lms,
      adaptive_state_lms,
      1,
      1.f);
...
float3 CompressPsychoV25ReferenceScaleHull(
    ...
    float peak_value,
    bool apply_display_map = true,
    int target_gamut_mode = 1) {
  ...
  float3 physical_compressed_lms = desired_lms;
  [branch]
  if (apply_display_map && white_curve_mode == 1) {
  ...
  // Intermediate passes retain PsychoV's Fast60 hue trajectory without
  // imposing a display-primary hull. The final display-map pass owns BT.2020
  // lower/upper-plane containment.
  if (!apply_display_map) {
    float3 hue_bent_lms = renodx::tonemap::psychov::psycho25_LMSFromAdaptiveMB(
        authored_mb,
        adaptive_state_lms);
    return hue_bent_lms * renodx::math::DivideSafe(
        authored_yf,
        renodx::tonemap::psychov::psycho25_YfFromLMS(hue_bent_lms),
        1.f);
  }
  ...
  // Uses the same Fast60 and Reference Scale hull pipeline as the full path,
  // with either source-direction recovery inside the hull or signed-opponent
  // retention after it. Shoulder-to-peak mapping remains in the later display
  // stage; the caller supplies that stage's peak for signed-opponent progress.
  float3 ApplyCustomPsychoV25ToneMapNoDisplayMap(
    float3 bt709_linear_input,
    float cone_response_exponent,
    float3 current_adaptive_state_bt709,
    float3 current_background_state_bt709,
    float hue_recovery_strength = 1.f,
    int hue_recovery_mode = PSYCHOV25_CUSTOM_HUE_RECOVERY_SOURCE_DIRECTION,
    float4 signed_opponent_retention_exponents = float4(
      1.5f,
      2.f,
      1.f,
      3.f),
    float peak_value = 1.f) {
    ...
    float3 source_lms = renodx::color::lms::from::BT709(finite_bt709_input);
    float3 adaptive_state_lms =
      renodx::color::lms::from::BT709(current_adaptive_state_bt709);
    float3 background_state_lms =
      renodx::color::lms::from::BT709(current_background_state_bt709);
    float3 target_lms_peak =
      renodx::color::lms::from::BT709(peak_value.xxx);
    ...
    float3 output_lms = CompressPsychoV25ReferenceScaleHull(
      ...
      background_state_lms,
      target_lms_peak,
      hue_recovery_mode == PSYCHOV25_CUSTOM_HUE_RECOVERY_SOURCE_DIRECTION
        ? hue_recovery_strength
        : 0.f,
      0.f,
      1.5f,
      0,
      ...
      false);
    [branch]
    if (hue_recovery_mode
          == PSYCHOV25_CUSTOM_HUE_RECOVERY_SIGNED_OPPONENT
        && hue_recovery_strength != 0.f) {
      output_lms =
        ApplyPsychoV25CustomSignedOpponentRetentionAndGamutCompression(
          contrast_lms,
          output_lms,
          adaptive_state_lms,
          background_state_lms,
          target_lms_peak,
          hue_recovery_strength,
          signed_opponent_retention_exponents,
          false);
    }
    return renodx::color::bt709::from::LMS(output_lms);
  }
...
float3 ApplyCustomPsychoV25ToneMap(
  ...
  float3 output_lms = CompressPsychoV25ReferenceScaleHull(
      ...
      target_gamut_mode);
  [branch]
  if (hue_recovery_mode == PSYCHOV25_CUSTOM_HUE_RECOVERY_SIGNED_OPPONENT
      && hue_recovery_strength != 0.f) {
    output_lms =
      ApplyPsychoV25CustomSignedOpponentRetentionAndGamutCompression(
        contrast_lms,
...
#endif  // RENODX_SHADERS_TONEMAP_PSYCHOV_TEST25_CUSTOM_HLSL_
