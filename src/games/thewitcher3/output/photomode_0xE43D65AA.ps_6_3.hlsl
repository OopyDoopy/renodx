...
float4 main(
  ...
  SaturationGrade grade = ApplySaturationGrade(SampleChromaticAberration(TEXCOORD, t0, s1).rgb);
  float3 gamma_color = ApplyUserColorGrading(grade);
  gamma_color = ApplyOutputRange(ApplyPhotoModeVignette(gamma_color, TEXCOORD_2));
