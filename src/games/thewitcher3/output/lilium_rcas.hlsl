...
#define ENABLE_NOISE_REMOVAL 1u
...
#define SHARPENING_NORMALIZATION_POINT 125
  ...
  float hitMinLum = renodx::math::DivideSafe(min4Lum, 4.f * limited_max4Lum, 0.f);
  float hitMaxLum = renodx::math::DivideSafe(peakC.x - limited_max4Lum, 4.f * min4Lum + peakC.y, 0.f);
...
#define FSR_RCAS_LIMIT 0.1875f
  ...
  nz = saturate(renodx::math::DivideSafe(abs(nz), maxLuma2x - minLuma2x, 0.f));
  nz = -0.5f * nz + 1.f;
  ...
  float3 pix = clamp(renodx::math::DivideSafe(pixLum, eLum, 1.f), 0.f, 4.f) * e;
