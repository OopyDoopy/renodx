...
float4 main(
  ...
  float _101 = saturate(_98 + kerare_brightness);
  CustomVignette(_101);
  _101 *= Exposure;
  if (_43) {
              ...
              do {
                ...
                do {
                  ...
                  if (RENODX_TONE_MAP_TYPE != 0.f
                    ...
                    const float3 lut_input = float3(_2074, _2075, _2076);
                    ...
                    const float3 lut_corrected = ApplySceneGradeLUT(
                        lut_input,
                        float3(_2479, _2480, _2481),
                        lut_black,
                        lut_mid);
                    _2479 = lut_corrected.x;
                    _2480 = lut_corrected.y;
                    _2481 = lut_corrected.z;
                  }
                  _2498 = max((mad(_2481, (fColorMatrix[2].x), mad(_2480, (fColorMatrix[1].x), (_2479 * (fColorMatrix[0].x)))) + (fColorMatrix[3].x)), 0.0f);
                  _2499 = max((mad(_2481, (fColorMatrix[2].y), mad(_2480, (fColorMatrix[1].y), (_2479 * (fColorMatrix[0].y)))) + (fColorMatrix[3].y)), 0.0f);
                  _2500 = max((mad(_2481, (fColorMatrix[2].z), mad(_2480, (fColorMatrix[1].z), (_2479 * (fColorMatrix[0].z)))) + (fColorMatrix[3].z)), 0.0f);
                } while (false);
              } while (false);
            } while (false);
          } while (false);
  ...
  float _2502 = min(_2499, 65000.0f);
  float _2503 = min(_2500, 65000.0f);
  if (!((cPassEnabled & 8) == 0)) {
    _2535 = (((cvdR.x * _2501) + (cvdR.y * _2502)) + (cvdR.z * _2503));
    _2536 = (((cvdG.x * _2501) + (cvdG.y * _2502)) + (cvdG.z * _2503));
    _2537 = (((cvdB.x * _2501) + (cvdB.y * _2502)) + (cvdB.z * _2503));
  } else {
    _2535 = _2501;
    _2536 = _2502;
    _2537 = _2503;
  ...
  if (!((cPassEnabled & 16) == 0)) {
    float _2549 = screenInverseSize.x * SV_Position.x;
    float _2550 = screenInverseSize.y * SV_Position.y;
    float4 _2553 = ImagePlameBase.SampleLevel(BilinearClamp, float2(_2549, _2550), 0.0f);
    float _2558 = _2553.x * ColorParam.x;
    float _2559 = _2553.y * ColorParam.y;
    float _2560 = _2553.z * ColorParam.z;
    float _2563 = ImagePlameAlpha.SampleLevel(BilinearClamp, float2(_2549, _2550), 0.0f);
    float _2568 = (_2553.w * ColorParam.w) * saturate((_2563.x * Levels_Rate) + Levels_Range);
    do {
      if (_2558 < 0.5f) {
        _2580 = ((_2535 * 2.0f) * _2558);
      } else {
        _2580 = (1.0f - (((1.0f - _2535) * 2.0f) * (1.0f - _2558)));
      }
      do {
        if (_2559 < 0.5f) {
          _2592 = ((_2536 * 2.0f) * _2559);
        } else {
          _2592 = (1.0f - (((1.0f - _2536) * 2.0f) * (1.0f - _2559)));
        }
        do {
          if (_2560 < 0.5f) {
            _2604 = ((_2537 * 2.0f) * _2560);
          } else {
            _2604 = (1.0f - (((1.0f - _2537) * 2.0f) * (1.0f - _2560)));
          }
          _2615 = (lerp(_2535, _2580, _2568));
          _2616 = (lerp(_2536, _2592, _2568));
          _2617 = (lerp(_2537, _2604, _2568));
        } while (false);
      } while (false);
    } while (false);
  } else {
  ...
  // if (!(useDynamicRangeConversionTemp == 0.0f)) {
  if (!(useDynamicRangeConversion == 0.0f) && RENODX_TONE_MAP_TYPE == 0) {
    // if (!(useHuePreserve == 0.0f)) {
    if (false) {
      float _2657 = mad((RGBToXYZViaCrosstalkMatrix[0].z), _2617, mad((RGBToXYZViaCrosstalkMatrix[0].y), _2616, ((RGBToXYZViaCrosstalkMatrix[0].x) * _2615)));
      float _2660 = mad((RGBToXYZViaCrosstalkMatrix[1].z), _2617, mad((RGBToXYZViaCrosstalkMatrix[1].y), _2616, ((RGBToXYZViaCrosstalkMatrix[1].x) * _2615)));
      float _2665 = (_2660 + _2657) + mad((RGBToXYZViaCrosstalkMatrix[2].z), _2617, mad((RGBToXYZViaCrosstalkMatrix[2].y), _2616, ((RGBToXYZViaCrosstalkMatrix[2].x) * _2615)));
      float _2666 = _2657 / _2665;
      float _2667 = _2660 / _2665;
      do {
        if (_2660 < curve_HDRip) {
          _2678 = (_2660 * exposureScale);
        } else {
          _2678 = ((log2((_2660 / curve_HDRip) - knee) * curve_k2) + curve_k4);
        }
        float _2680 = (_2666 / _2667) * _2678;
        float _2684 = (((1.0f - _2666) - _2667) / _2667) * _2678;
        _2733 = min(max(mad((XYZToRGBViaCrosstalkMatrix[0].z), _2684, mad((XYZToRGBViaCrosstalkMatrix[0].y), _2678, (_2680 * (XYZToRGBViaCrosstalkMatrix[0].x)))), 0.0f), 65536.0f);
        _2734 = min(max(mad((XYZToRGBViaCrosstalkMatrix[1].z), _2684, mad((XYZToRGBViaCrosstalkMatrix[1].y), _2678, (_2680 * (XYZToRGBViaCrosstalkMatrix[1].x)))), 0.0f), 65536.0f);
        _2735 = min(max(mad((XYZToRGBViaCrosstalkMatrix[2].z), _2684, mad((XYZToRGBViaCrosstalkMatrix[2].y), _2678, (_2680 * (XYZToRGBViaCrosstalkMatrix[2].x)))), 0.0f), 65536.0f);
      } while (false);
    } else {
      do {
        if (_2615 < curve_HDRip) {
          _2711 = (exposureScale * _2615);
        } else {
          _2711 = ((log2((_2615 / curve_HDRip) - knee) * curve_k2) + curve_k4);
        }
        do {
          if (_2616 < curve_HDRip) {
            _2722 = (exposureScale * _2616);
          } else {
            _2722 = ((log2((_2616 / curve_HDRip) - knee) * curve_k2) + curve_k4);
          }
          if (_2617 < curve_HDRip) {
            _2733 = _2711;
            _2734 = _2722;
            _2735 = (exposureScale * _2617);
          } else {
            _2733 = _2711;
            _2734 = _2722;
            _2735 = ((log2((_2617 / curve_HDRip) - knee) * curve_k2) + curve_k4);
          }
        } while (false);
      } while (false);
    }
