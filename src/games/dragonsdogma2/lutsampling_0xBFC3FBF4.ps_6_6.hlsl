...
float4 CustomLutSampling(float2 uv) {
  ...
  if (RENODX_TONE_MAP_TYPE == 0.f) {
    source.xyz *= whitePaperNits * 0.009999999776482582f;
  ...
  if (RENODX_TONE_MAP_TYPE == 2.f) {
    float psycho_peak = RENODX_GAMMA_CORRECTION > 0.f
        ? GammaCorrectionLMS(calculated_peak, true)
        : calculated_peak;
    ...
    output_color = min(renodx::tonemap::psycho::psychotm_customized(ungraded_bt709, psycho_config), psycho_peak);
  } else if (RENODX_TONE_MAP_TYPE == 1.f) {
    return float4(ApplyCustomACESToneMapEncodePQ(
        source.rgb,
        RENODX_PEAK_WHITE_NITS,
        RENODX_DIFFUSE_WHITE_NITS * 1.5f,
        uv), 1.f);
  }

  output_color = CustomPostProcessing(output_color, uv);
  if (RENODX_GAMMA_CORRECTION > 0.f) {
    output_color = RENODX_GAMMA_CORRECTION == 1.f
        ? renodx::color::correct::GammaSafe(output_color)
        : GammaCorrectionLMS(output_color);
  }
  return float4(renodx::color::pq::EncodeSafe(
      renodx::color::bt2020::from::BT709(output_color), RENODX_DIFFUSE_WHITE_NITS), 1.f);
}
...
float4 main(
  ...
  return CustomLutSampling(TEXCOORD);
