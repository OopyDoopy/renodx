...
float3 ApplyToneMapEncodePQ(float3 untonemapped_ap1, float cbuffer_peak_nits, float cbuffer_diffuse_white_nits, float2 uv) {
  ...
  if (RENODX_TONE_MAP_SCALING == 0.f) {
    ...
    float tonemapped_bt2020_yf = renodx::color::yf::from::BT2020(tonemapped_bt2020);
    tonemapped_bt2020 = max(0, renodx::color::correct::Luminance(tonemapped_bt2020, tonemapped_bt2020_yf, tonemapped_yf));
  }
