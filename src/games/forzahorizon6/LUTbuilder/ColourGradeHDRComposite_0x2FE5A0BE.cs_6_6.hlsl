...
float3 ImprovedColorGradingLUTs(float3 input_color, float lut_scaling = 1.f) {
  ...
  if (lut_scaling > 0.f) {
    ...
    if (renodx::math::Min(lut_black) > black_floor) {
      ...
      if (RENODX_GAMMA_CORRECTION != 0.f && RENODX_IS_HDR_COLOR_SPACE != 0.f) {
        lut_corrected_lms = renodx::color::correct::GammaSafe(lut_corrected_lms);
        reference_lms = renodx::color::correct::GammaSafe(reference_lms);
        lut_black_lms = renodx::color::correct::GammaSafe(lut_black_lms);
        lut_mid_lms = renodx::color::correct::GammaSafe(lut_mid_lms);
      }

      lut_corrected_lms = renodx::color::srgb::EncodeSafe(lut_corrected_lms);
      reference_lms = renodx::color::srgb::EncodeSafe(reference_lms);
      lut_black_lms = renodx::color::srgb::EncodeSafe(lut_black_lms);
      lut_mid_lms = renodx::color::srgb::EncodeSafe(lut_mid_lms);
...
void comp_main() {
  ...
  if (RENODX_TONE_MAP_TYPE == 1.f) {
    ...
    const float tone_map_highlights = RENODX_TONE_MAP_HIGHLIGHTS;
    const float tone_map_shadows = RENODX_TONE_MAP_SHADOWS;
    float tone_map_contrast = RENODX_TONE_MAP_CONTRAST;
    ...
    float mid_gray_in = RENODX_TONE_MAP_MID_GRAY_IN;
    ...
    float3 working_color_hdr = renodx::tonemap::psycho::psychotm_customized(
        working_color,
        display_peak_scale,
        tone_map_exposure,
        tone_map_highlights,
    ...
    float3 working_color_sdr = renodx::tonemap::psycho::psychotm_customized(
        working_color,
        1.f,
        tone_map_exposure,
        tone_map_highlights,
