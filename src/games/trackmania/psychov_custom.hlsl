...
#ifndef SRC_TRACKMANIA_PSYCHOV_CUSTOM_HLSL_
#define SRC_TRACKMANIA_PSYCHOV_CUSTOM_HLSL_
...
Config Create() {
  ...
  config.uc2_e = 0.01f;
  config.uc2_f = 0.30f;
  config.uc2_pivot = 1.f;
  config.uc2_white_precompute = 1.f;
  return config;
}
...
float3 psychotm_customized(
  ...
  if (grading_mode == 0) {
    ...
    if (shadows != 1.f) {
      ...
      float3 shadowed = pow(scaled, 2.f - shadows);
  ...
  // Extend the game curve after PsychoV's contrast/highlight/shadow/flare
  // grading, while the signal is still in LMS space. The PsychoV white curve
  // remains a separate downstream display-mapping stage.
  float3 uc2_base = renodx::tonemap::ApplyCurve(
      lms_graded,
      psycho_config.uc2_a,
      psycho_config.uc2_b,
      psycho_config.uc2_c,
      ...
      psycho_config.uc2_e,
      psycho_config.uc2_f)
      * psycho_config.uc2_white_precompute;
  lms_graded = Uncharted2::ApplyExtended(
      lms_graded,
      uc2_base,
      psycho_config.uc2_pivot,
      psycho_config.uc2_white_precompute,
      psycho_config.uc2_a,
      psycho_config.uc2_b,
      psycho_config.uc2_c,
...
float3 ApplyGrading(float3 bt709_linear, renodx::draw::Config config, float3 peak_value = 1.f, float current_adaptive_state_bt709 = 0.18f, float current_background_state_bt709 = 0.18f, int grading_mode = 0) {
...
#endif  // SRC_TRACKMANIA_PSYCHOV_CUSTOM_HLSL_
