...
renodx::utils::settings::Settings settings = {
    ...
    new renodx::utils::settings::Setting{
        ...
        .binding = &SCENE_GRADE_COLOR_GRADE_STRENGTH,
    ...
    new renodx::utils::settings::Setting{
        ...
        .binding = &SCENE_GRADE_COLOR_GRADE_STRENGTH_TWO,
        .value_type = renodx::utils::settings::SettingValueType::FLOAT,
        .default_value = 100.f,
        .label = "LUT Strength",
        .section = "Scene Grading",
        .tooltip = "Adjust the strength of the LUT grading pass.",
        .max = 100.f,
        .is_enabled = []() { return shader_injection.tone_map_type != 0; },
        .parse = [](float value) { return value * 0.01f; },
        .is_visible = []() { return settings[0]->GetValue() >= 1; },
...
BOOL APIENTRY DllMain(HMODULE h_module, DWORD fdw_reason, LPVOID lpv_reserved) {
  switch (fdw_reason) {
    case DLL_PROCESS_ATTACH:
      ...
      // Full-resolution scene targets that are copied later need to be upgraded at creation time.
      // Clone-only upgrades leave the original B8 resource behind, so CopyResource still sees the
      // original source/destination handles (for example menu preview copies into temporary memory).
      ...
      // Main scene/menu targets. Keep this constrained to render targets so static menu textures
      // (for example 1024x1024 B8G8R8A8 uploads) are not upgraded.
      renodx::mods::swapchain::resource_upgrade_infos.push_back({
          .old_format = reshade::api::format::b8g8r8a8_typeless,
          .new_format = reshade::api::format::r16g16b16a16_float,
      ...
      // Some full-resolution scene/menu history resources are only observed as shader resources
      // in the broken views. Keep this constrained to the backbuffer size instead of ignore_size.
      renodx::mods::swapchain::resource_upgrade_infos.push_back({
          .old_format = reshade::api::format::b8g8r8a8_typeless,
          .new_format = reshade::api::format::r16g16b16a16_float,
          .use_resource_view_cloning = true,
      ...
      // Final full-resolution SDR color source sampled into the HDR10 output path in menu views.
      renodx::mods::swapchain::resource_upgrade_infos.push_back({
      ...
      // Half-resolution menu/UI intermediates seen at 1720x720 on a 3440x1440 backbuffer.
      renodx::mods::swapchain::resource_upgrade_infos.push_back({
          .old_format = reshade::api::format::b8g8r8a8_typeless,
          .new_format = reshade::api::format::r16g16b16a16_float,
          .use_resource_view_cloning = true,
          .dimensions = {
              .width = static_cast<int16_t>(buffer_width / 2.f),
              .height = static_cast<int16_t>(buffer_height / 2.f),
          },
          .usage_include = reshade::api::resource_usage::render_target,
          .use_resource_view_cloning_and_upgrade = true,
      });

      // Some menu/world preview views are rendered to a non-backbuffer-sized typeless target.
      renodx::mods::swapchain::resource_upgrade_infos.push_back({
          .old_format = reshade::api::format::b8g8r8a8_typeless,
          .new_format = reshade::api::format::r16g16b16a16_float,
          .use_resource_view_cloning = true,
          .dimensions = {.width = 1200, .height = 1080},
          .usage_include = reshade::api::resource_usage::render_target,
          .use_resource_view_cloning_and_upgrade = true,
      });

      // Half-resolution scene views in menus can be created as SRGB render targets instead of typeless.
      renodx::mods::swapchain::resource_upgrade_infos.push_back({
      ...
      // Conditional UI/content target. Only upgrade it when it is created as a render target;
      // uploaded 1024x512/1024x1024 menu textures must remain untouched.
      renodx::mods::swapchain::resource_upgrade_infos.push_back({
          .old_format = reshade::api::format::b8g8r8a8_typeless,
          .new_format = reshade::api::format::r16g16b16a16_float,
          .use_resource_view_cloning = true,
