#ifndef SRC_TRACKMANIA_UNCHARTED2EXTENDED_HLSLI_
#define SRC_TRACKMANIA_UNCHARTED2EXTENDED_HLSLI_
...
//     // Î” â‰¥ 0 â†’ one real root, cubeâ€‘root form
//     float sqrt_delta = sqrt(delta);
//     z = pow(-q / 2.f + sqrt_delta, 1.f / 3.f) + pow(-q / 2.f - sqrt_delta, 1.f / 3.f);
//   } else {
//     // Î” < 0 â†’ three real roots, use cosine form
...
float FindSecondDerivativeRoot(float a, float b, float c, float d, float e, float f) {
  ...
  // If A3 = 0, curve is degenerate â†’ no inflection
  if (abs(A3) < 1e-12f)
    return 0.f;
...
// FXC/SM5-safe numerical replacement for the analytic quartic knee solver.
// Search [0, 1] for sign changes in f''' and refine the largest root, matching
// the saturated root selected by FindThirdDerivativeRoot.
float FindThirdDerivativeRootSM5(
    float a, float b, float c,
    float d, float e, float f) {
  static const int kSteps = 16;
  static const int kRefinements = 8;

  float root = 1.f;
  float previous_x = 0.f;
  float previous_value = ThirdDerivative(previous_x, a, b, c, d, e, f);
  bool found = false;

  [unroll]
  for (int i = 1; i <= kSteps; ++i) {
    float current_x = float(i) / float(kSteps);
    float current_value = ThirdDerivative(current_x, a, b, c, d, e, f);

    if (previous_value * current_value <= 0.f) {
      float low = previous_x;
      float high = current_x;
      float low_value = previous_value;

      [unroll]
      for (int j = 0; j < kRefinements; ++j) {
        float middle = (low + high) * 0.5f;
        float middle_value = ThirdDerivative(middle, a, b, c, d, e, f);
        if (low_value * middle_value <= 0.f) {
          high = middle;
        } else {
          low = middle;
...
#endif  // SRC_TRACKMANIA_UNCHARTED2EXTENDED_HLSLI_
